create_graphs <- function(network_file, link_type = c("all","retweet","reply","quoted","mentions")){
   ...
   if(type == "all"){
      ...
      network_file.mentions.graph <- graph_from_data_frame(network_file.mentions, directed = TRUE)
...
files <- files[1:3]
for(graph in 1:length(files)){
   files[[graph]] <- create_graphs(files[[graph]], link_type = "all")
}
